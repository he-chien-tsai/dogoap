...
for EXAMPLE in "${EXAMPLES[@]}"; do
  ...
  RUSTFLAGS="-Zlocation-detail=none" cargo build --profile=wasm-release --no-default-features --features=bevy_winit --example="$EXAMPLE" --target=wasm32-unknown-unknown -Z build-std-features=panic_immediate_abort -Z build-std=std,panic_abort -Z build-std-features="optimize_for_size"
